...
if ( typeof ToggleGroup !== 'function' ) {
  ...
  var faqTitles = document.querySelectorAll('.home_faq_wrapper');
  var defaultTab = faqTitles[0];
  defaultTab.classList.add('active');
  var defaultTabContent = defaultTab.querySelector('.faq_content');
  defaultTabContent.style.maxHeight = '1000px';
  faqTitles.forEach((title) => {
    title.addEventListener('click', function() {
      var isActive = this.classList.contains('active');
      faqTitles.forEach((t) => {
        t.classList.remove('active');
        var faqContents = t.querySelector('.faq_content');
        faqContents.style.maxHeight = '0px';
      });
      if (!isActive) {
        this.classList.add('active');
        var faqContents = this.querySelector('.faq_content');
        faqContents.style.maxHeight = '1000px';
      }
    });
  });
}
